...
# IMPORTANT: This module uses ERPNext's standard get_conversion_factor function to ensure
# proper UOM conversion factor calculation instead of hardcoding conversion_factor = 1.0.
# This follows ERPNext's best practices for handling different UOMs and their conversions.
...
def aggregate_orders_and_create_aggregate_orders(branches: List[str], order_date: str) -> Dict[str, Any]:
    """
    Main function to aggregate orders from SF Order Master for specific branches and date,
    then create cyclic SF Aggregate Order Detail documents based on plant -> distribution center -> darkstore hierarchy.
    ...
    Returns:
        Dict with aggregation results and created aggregate orders
    """
    ...
    try:
        ...
        # Step 5: Create cyclic aggregate orders
        # Combine D2C and B2B for internal transfers, but separate for customer deliveries
        
        # Ensure any pending transactions from error logging are committed before aggregate order creation
        try:
            if hasattr(frappe.db, 'transaction_writes') and frappe.db.transaction_writes > 0:
                frappe.db.commit()
                debug_print("Committed pending transactions before aggregate order creation")
        except Exception as e:
            debug_print(f"No pending transactions to commit: {str(e)}")
        
        created_aggregate_orders = create_combined_aggregate_orders(
            processed_d2c_orders, processed_b2b_orders, order_date
        )
        
        total_created_orders = len(created_aggregate_orders)
        info_print(f"Created {total_created_orders} aggregate orders (D2C: {len(created_aggregate_orders['d2c_orders'])}, B2B: {len(created_aggregate_orders['b2b_orders'])})")
        ...
        return {
            ...
            "created_aggregate_orders": created_aggregate_orders,
            "message": f"Successfully processed {total_processed_orders} orders and created {total_created_orders} aggregate orders"
        }
...
def create_combined_aggregate_orders(d2c_orders: Dict, b2b_orders: Dict, order_date: str) -> Dict:
    """
    Create combined SF Aggregate Order Detail documents for both D2C and B2B orders
    Combines D2C and B2B for internal transfers, separates for customer deliveries
    """
    info_print("Creating combined aggregate orders for D2C and B2B orders")
    ...
    try:
        ...
        for plant, plant_data in combined_orders.items():
            for dc, dc_data in plant_data.items():
                ...
                dc_order = create_aggregate_order_for_distribution_center(
                    plant, dc, dc_data, order_date, internal_customer, default_company, True, "Internal"
                )
                if dc_order:
                    created_orders.append(dc_order)
                    info_print(f"Created combined DC order: {dc_order['aggregate_order']}")
                ...
                for darkstore, darkstore_data in dc_data.items():
                    if darkstore == "DC_DIRECT":
                        ...
                        if b2b_orders_list:
                            ...
                            for customer, customer_orders in customer_groups.items():
                                if customer != internal_customer:
                                    dc_direct_order = create_aggregate_order_for_dc_direct_with_orders(
                                        dc, customer_orders, order_date, customer, default_company, False
                                    )
                                    if dc_direct_order:
                                        created_orders.append(dc_direct_order)
                                        info_print(f"Created DC direct order: {dc_direct_order['aggregate_order']}")
                        ...
                        if b2b_orders_list:
                            ...
                            for customer, customer_orders in customer_groups.items():
                                if customer != internal_customer:
                                    plant_direct_order = create_aggregate_order_for_plant_direct_with_orders(
                                    ...
                                    if plant_direct_order:
                                        ...
                                        info_print(f"Created plant direct order: {plant_direct_order['aggregate_order']}")
                    else:
                        # Regular darkstore orders
                        # Create ONE internal transfer (DC to Darkstore) with ALL items
                        if darkstore_data.get("items"):
                            darkstore_order = create_aggregate_order_for_darkstore(
                            ...
                            if darkstore_order:
                                ...
                                info_print(f"Created combined darkstore order: {darkstore_order['aggregate_order']}")
                        ...
                        if b2b_orders_list:
                            ...
                            for customer, customer_orders in customer_groups.items():
                                if customer != internal_customer:
                                    darkstore_to_client_order = create_aggregate_order_for_darkstore_to_client_with_orders(
                                    ...
                                    if darkstore_to_client_order:
                                        ...
                                        info_print(f"Created darkstore to client order: {darkstore_to_client_order['aggregate_order']}")
        ...
        # Remove rollback - no transaction was started at this level
        # frappe.db.rollback()  # REMOVED
        error_print(f"Error in create_combined_aggregate_orders: {str(e)}")
        log_error(
            error_category="Order Processing",
            error_description=f"Failed to create combined aggregate orders: {str(e)}",
            processing_stage="Order Aggregation",
            entity_type="SF Aggregate Order Detail",
            error_severity="Critical",
            additional_detail={
                "order_date": order_date,
                "error": str(e)
...
def create_aggregate_order_for_dc_direct_with_orders(dc_warehouse: str, customer_orders: List[Dict], order_date: str,
                                                   customer: str, company: str, is_internal: bool) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for direct DC to client with specific orders
    """
    ...
    try:
        debug_print(f"Creating direct DC to client aggregate order for {dc_warehouse} with {len(customer_orders)} orders")
        ...
        aggregate_order_data = {
            "doctype": "SF Aggregate Order Detail",
            "customer": customer,
            "transaction_date": order_date,
            "delivery_date": add_days(order_date, 1),
            "company": company,
        ...
        if customer_address:
            aggregate_order_data["shipping_address_name"] = customer_address
        
        # Only set target warehouse for internal customers
        if is_internal:
            aggregate_order_data["set_target_warehouse"] = dc_warehouse
        
        aggregate_order = frappe.get_doc(aggregate_order_data)
        
        # Add aggregated items
        for item_code, quantity in aggregated_items.items():
            if quantity > 0:
                # Get item details for required fields
                item_details = get_item_details_for_aggregate_order(item_code)
                
                aggregate_order.append("items", {
                    "item_code": item_code,
                    "item_name": item_details["item_name"],
                    "qty": quantity,
                    "uom": item_details["uom"],
                    "conversion_factor": item_details["conversion_factor"],
        ...
        if not aggregate_order.items:
            debug_print(f"No items to add to DC direct aggregate order for {dc_warehouse}")
            if transaction_started:
                frappe.db.rollback()
            return None
            
        aggregate_order.insert()
        aggregate_order.submit()
        
        # Commit the transaction only if we started it
        if transaction_started:
            frappe.db.commit()
        
        debug_print(f"Successfully created DC direct aggregate order {aggregate_order.name}")
        
        return {
            "type": "dc_direct_order",
            "order_type": "B2B",
            "aggregate_order": aggregate_order.name,
            "from_warehouse": dc_warehouse,
            "to_customer": customer,
            "items_count": len(aggregate_order.items),
            "total_qty": sum(item.qty for item in aggregate_order.items)
        }
        ...
        if transaction_started:
            frappe.db.rollback()
        error_print(f"Error creating DC direct aggregate order: {str(e)}")
        log_error(
            error_category="Order Processing",
            error_description=f"Failed to create DC direct aggregate order: {str(e)}",
            processing_stage="Order Aggregation",
            entity_type="SF Aggregate Order Detail",
            reference_doctype="SF Aggregate Order Detail",
            error_severity="High",
            additional_detail={
                "from_warehouse": dc_warehouse,
                "to_customer": customer,
...
def create_aggregate_order_for_plant_direct_with_orders(plant_warehouse: str, customer_orders: List[Dict], order_date: str,
                                                      customer: str, company: str, is_internal: bool) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for direct plant to client with specific orders
    """
    # Check if we're already in a transaction
    in_transaction = hasattr(frappe.db, 'transaction_writes') and frappe.db.transaction_writes > 0
    transaction_started = False
    ...
    try:
        debug_print(f"Creating direct plant to client aggregate order for {plant_warehouse} with {len(customer_orders)} orders")
        ...
        # Only set target warehouse for internal customers
        if is_internal:
            aggregate_order_data["set_target_warehouse"] = plant_warehouse
        ...
        if not aggregate_order.items:
            debug_print(f"No items to add to plant direct aggregate order for {plant_warehouse}")
            if transaction_started:
                frappe.db.rollback()
            return None
        ...
        debug_print(f"Successfully created plant direct aggregate order {aggregate_order.name}")
        ...
        if transaction_started:
            frappe.db.rollback()
        error_print(f"Error creating plant direct aggregate order: {str(e)}")
        log_error(
            error_category="Order Processing",
            error_description=f"Failed to create plant direct aggregate order: {str(e)}",
            processing_stage="Order Aggregation",
            entity_type="SF Aggregate Order Detail",
            reference_doctype="SF Aggregate Order Detail",
            error_severity="High",
...
def create_aggregate_order_for_darkstore_to_client_with_orders(darkstore_warehouse: str, customer_orders: List[Dict], order_date: str,
                                                             customer: str, company: str, is_internal: bool) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for Darkstore to Client with specific orders
    """
    # Check if we're already in a transaction
    in_transaction = hasattr(frappe.db, 'transaction_writes') and frappe.db.transaction_writes > 0
    transaction_started = False
    ...
    try:
        debug_print(f"Creating darkstore to client aggregate order for {darkstore_warehouse} with {len(customer_orders)} orders")
        ...
        # Only set target warehouse for internal customers
        if is_internal:
            aggregate_order_data["set_target_warehouse"] = darkstore_warehouse
        ...
        if not aggregate_order.items:
            debug_print(f"No items to add to darkstore to client aggregate order for {darkstore_warehouse}")
            if transaction_started:
                frappe.db.rollback()
            return None
        ...
        debug_print(f"Successfully created darkstore to client aggregate order {aggregate_order.name}")
        ...
        if transaction_started:
            frappe.db.rollback()
        error_print(f"Error creating darkstore to client aggregate order: {str(e)}")
        log_error(
            error_category="Order Processing",
            error_description=f"Failed to create darkstore to client aggregate order: {str(e)}",
            processing_stage="Order Aggregation",
            entity_type="SF Aggregate Order Detail",
            reference_doctype="SF Aggregate Order Detail",
            error_severity="High",
...
def create_aggregate_order_for_darkstore(dc_warehouse: str, darkstore_warehouse: str, 
                                       darkstore_data: Dict, order_date: str, 
                                       customer: str, company: str, is_internal: bool, order_type: str) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for darkstore (from distribution center to darkstore)
    """
    # Check if we're already in a transaction
    in_transaction = hasattr(frappe.db, 'transaction_writes') and frappe.db.transaction_writes > 0
    transaction_started = False
    ...
    try:
        debug_print(f"Creating aggregate order for darkstore {darkstore_warehouse}")
        ...
        if shipping_address:
            aggregate_order_data["shipping_address_name"] = shipping_address
        ...
        if not aggregate_order.items:
            debug_print(f"No items to add to darkstore aggregate order for {darkstore_warehouse}")
            if transaction_started:
                frappe.db.rollback()
            return None
        ...
        debug_print(f"Successfully created darkstore aggregate order {aggregate_order.name}")
        ...
        if transaction_started:
            frappe.db.rollback()
        error_print(f"Error creating darkstore aggregate order: {str(e)}")
        log_error(
            error_category="Order Processing",
            error_description=f"Failed to create darkstore aggregate order: {str(e)}",
            processing_stage="Order Aggregation",
            entity_type="SF Aggregate Order Detail",
            reference_doctype="SF Aggregate Order Detail",
            error_severity="High",
...
def create_aggregate_order_for_distribution_center(plant_warehouse: str, dc_warehouse: str,
                                                 dc_data: Dict, order_date: str,
                                                 customer: str, company: str, is_internal: bool, order_type: str) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for distribution center (from plant to distribution center)
    """
    # Check if we're already in a transaction
    in_transaction = hasattr(frappe.db, 'transaction_writes') and frappe.db.transaction_writes > 0
    transaction_started = False
    ...
    try:
        debug_print(f"Creating aggregate order for distribution center {dc_warehouse}")
        ...
        if not aggregate_order.items:
            debug_print(f"No items to add to DC aggregate order for {dc_warehouse}")
            if transaction_started:
                frappe.db.rollback()
            return None
        ...
        debug_print(f"Successfully created DC aggregate order {aggregate_order.name}")
        ...
        if transaction_started:
            frappe.db.rollback()
        error_print(f"Error creating distribution center aggregate order: {str(e)}")
        log_error(
            error_category="Order Processing",
            error_description=f"Failed to create distribution center aggregate order: {str(e)}",
            processing_stage="Order Aggregation",
            entity_type="SF Aggregate Order Detail",
            reference_doctype="SF Aggregate Order Detail",
            error_severity="High",
...
def create_aggregate_order_for_dc_direct(dc_warehouse: str, dc_data: Dict, order_date: str,
                                       customer: str, company: str, is_internal: bool) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for direct DC to client (B2B orders)
    """
    try:
        debug_print(f"Creating direct DC to client aggregate order for {dc_warehouse}")
...
def create_aggregate_order_for_plant_direct(plant_warehouse: str, plant_data: Dict, order_date: str,
                                          customer: str, company: str, is_internal: bool) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for direct plant to client (B2B orders)
    """
    try:
        debug_print(f"Creating direct plant to client aggregate order for {plant_warehouse}")
...
def create_aggregate_order_for_darkstore_to_client(darkstore_warehouse: str, darkstore_data: Dict, order_date: str,
                                                   customer: str, company: str, is_internal: bool) -> Optional[Dict]:
    """
    Create SF Aggregate Order Detail for Darkstore to Client (B2B orders)
    This creates the final aggregate order from Darkstore directly to the B2B client
    """
    try:
        debug_print(f"Creating darkstore to client aggregate order for {darkstore_warehouse}")
...
def get_item_details_for_aggregate_order(item_code: str, uom: str = None) -> Dict[str, Any]:
    """
    Get required item details for creating aggregate order items
    Follows ERPNext's standard get_item_details pattern
    
    Args:
        item_code: ERPNext Item code
        uom: Specific UOM to use (optional, will use default if not provided)
    
    Returns:
        Dict with item details including item_name, uom, conversion_factor, stock_uom
    """
    try:
        ...
        if uom:
            # Use the specified UOM
            selected_uom = uom
        else:
            # Use ERPNext's logic: prefer sales_uom, then stock_uom
            selected_uom = item_doc.sales_uom if item_doc.sales_uom else item_doc.stock_uom
        
        # Calculate conversion factor using ERPNext's standard method
        if item_doc.stock_uom == selected_uom:
            conversion_factor = 1.0
        else:
            # Use ERPNext's get_conversion_factor function
            try:
                conversion_factor_result = frappe.call(
                    "erpnext.stock.get_item_details.get_conversion_factor",
                    item_code=item_code,
                    uom=selected_uom
                )
                conversion_factor = conversion_factor_result.get("conversion_factor", 1.0)
            except Exception as conv_error:
                error_print(f"Error getting conversion factor for {item_code} with UOM {selected_uom}: {str(conv_error)}")
                # Fallback to 1.0 if conversion factor lookup fails
                conversion_factor = 1.0
        ...
        error_print(f"Error getting item details for {item_code}: {str(e)}")
        log_error(
            error_category="Missing Reference",
            error_description=f"Error getting item details for {item_code}: {str(e)}",
            processing_stage="Item Validation",
            entity_type="Product",
            external_id=item_code,
            reference_doctype="Item",
            internal_reference=item_code,
            error_severity="High",
            additional_detail={
                "item_code": item_code,
                "uom": uom,
...
def test_aggregate_orders(branches_json: str, order_date: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result = aggregate_orders_and_create_aggregate_orders(branches, order_date)
...
def daily_order_aggregation():
    ...
    try:
        ...
        result = aggregate_orders_and_create_aggregate_orders(branch_names, current_date)
...
# bench execute "inv_mgmt.cron_functions.aggregate_order_data_sf.daily_order_aggregation"